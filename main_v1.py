...
# # from scripts.student_projects import projectProjectManager
...
# #     python main.py --create-project-repos --projects config/project_data.csv
...
# #     parser.add_argument('--projects', help='project data CSV file path')
...
# #     parser.add_argument('--create-project-projects', action='store_true',
# #                        help='Create individual project project boards')
# #     parser.add_argument('--send-invitations', action='store_true',
# #                        help='Send invitations to projects and supervisors')
...
# #     parser.add_argument('--retry-failed', help='Retry failed operations from file')
...
# #             args.full_setup, args.create_project_repos, args.create_student_projects,
# #             args.send_invitations, args.update_progress, args.generate_analytics,
# #             args.weekly_report, args.risk_assessment, args.check_status
# #         ]
...
# #             if not args.force:
# #                 response = input("This will create master project, project repositories, and send invitations. Continue? (y/N): ")
# #                 if response.lower() != 'y':
...
# #                 print("\n1. Creating master project...")
# #                 master_manager = MasterProjectManager(config)
# #                 master_result = master_manager.create_master_project()
# #                 print(f"Master project created: {master_result.get('project_name', 'N/A')}")
                
# #                 # Step 2: Create project repositories
# #                 print("\n2. Creating project repositories...")
# #                 repo_results = bulk_processor.bulk_create_repositories(args.projects, progress_callback)
# #                 print_results_summary(repo_results, "Repository Creation")
                
# #                 # Step 3: Send invitations
# #                 print("\n\n3. Sending invitations...")
# #                 invitation_results = bulk_processor.bulk_send_invitations(args.projects, progress_callback)
# #                 print_results_summary(invitation_results, "Invitation Sending")
                
# #                 # Step 4: Update master dashboard
# #                 print("\n\n4. Updating master dashboard...")
# #                 progress_aggregator = ProgressAggregator(config)
# #                 progress_data = progress_aggregator.collect_all_progress(args.projects)
# #                 master_manager.update_master_dashboard(master_result['project_id'], progress_data)
                
# #                 print("\n=== Full Setup Completed Successfully! ===")
# #                 print(f"Master project: {config.get('project', {}).get('main_project_name')}")
# #                 print(f"project repositories: {len(repo_results.get('successful', []))}")
# #                 print(f"Invitations sent: {len(invitation_results.get('successful', []))}")
...
# #             results = bulk_processor.bulk_create_repositories(args.projects, progress_callback)
# #             print_results_summary(results, "Repository Creation")
        
# #         if args.create_student_projects:
# #             print("Creating project project boards...")
# #             project_manager = projectProjectManager(config)
# #             results = project_manager.create_all_student_projects(args.projects)
# #             print_results_summary(results, "project Project Creation")
        
# #         if args.send_invitations:
...
# #             results = analytics_generator.generate_comprehensive_analytics(args.projects)
# #             print(f"\nAnalytics report generated: {results.get('report_id', 'N/A')}")
# #             print(f"Total projects analyzed: {results.get('overview', {}).get('total_projects', 0)}")
# #             print(f"Average progress: {results.get('overview', {}).get('average_progress', 0):.1f}%")
...
# #             print(f"\n=== Risk Assessment ===")
# #             print(f"High risk projects: {len(risk_data.get('high_risk', []))}")
# #             print(f"Medium risk projects: {len(risk_data.get('medium_risk', []))}")
# #             print(f"Low risk projects: {len(risk_data.get('low_risk', []))}")
...
# #             print(f"Retrying failed operations from {args.retry_failed}...")
# #             # This would load failed results and retry them
# #             print("Retry functionality not implemented in this example")
...
#                     response = input("This will use existing master project, create project repositories, and send invitations. Continue? (y/N): ")
#                 else:
#                     response = input("This will create master project, project repositories, and send invitations. Continue? (y/N): ")
#                 if response.lower() != 'y':
#                     print("Operation cancelled.")
...
#                 # Step 1: Create or use existing master project
#                 if args.use_existing_master:
#                     print("\n1. Using existing master project...")
#                     master_manager = MasterProjectManager(config)
#                     # Try to get existing project info
#                     try:
#                         master_result = master_manager.get_existing_project()
#                         print(f"Using existing master project: {master_result.get('project_name', 'N/A')}")
#                     except Exception as e:
#                         print(f"Warning: Could not fetch existing project info: {e}")
#                         # Create a minimal result to continue
#                         master_result = {
#                             'project_name': config.get('project', {}).get('main_project_name'),
#                             'project_id': None  # Will be fetched later if needed
#                         }
#                 else:
#                     print("\n1. Creating master project...")
#                     master_manager = MasterProjectManager(config)
...
#                 # Step 4: Update master dashboard (if project_id is available)
#                 if master_result and master_result.get('project_id'):
#                     print("\n\n4. Updating master dashboard...")
#                     progress_aggregator = ProgressAggregator(config)
#                     progress_data = progress_aggregator.collect_all_progress(args.projects)
#                     master_manager.update_master_dashboard(master_result['project_id'], progress_data)
#                 else:
#                     print("\n\n4. Skipping master dashboard update (project ID not available)")
...
Examples:
  ...
  Operation management:
    ...
    try:
        ...
        # Check if project data is required and provided
        operations_requiring_projects = [
            args.full_setup, args.create_project_repos, args.create_student_folders,
        ...
        if args.full_setup:
            ...
            try:
                # Step 1: Create or setup repository
                ...
                # Step 3: Create GitHub Project for master dashboard
                print("\n3. Creating master project board...")
                master_manager = MasterProjectManager(config)
                project_result = master_manager.create_repository_project(args.projects)
                
                # Step 4: Create student issues
                ...
                print("\n=== Full Setup Completed Successfully! ===")
                print(f"Master repository: {config.get('github', {}).get('organization', '')}/{config.get('repository', {}).get('name', '')}")
